...
def update_plot():
    ...
    librosa.display.specshow(db, sr=sr, hop_length=HOP_LENGTH, y_axis="hz", ax=ax, y_coords=None)
...
voice_quality_spinbox = tk.Spinbox(
    input_frame, from_=0, to=3, textvariable=voice_quality_var, width=10, command=on_voice_quality_change
)
...
interview_check = ttk.Checkbutton(
    input_frame, text="Interview", variable=interview_var, command=on_interview_change, onvalue=True, offvalue=False
)
